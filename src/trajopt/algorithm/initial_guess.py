...
def straight_line_initial_guess(params):
    ...
    # Initial state
    zs_init             = np.array([np.linspace(params["mission"]["zi"][i], params["mission"]["zf"][i], params["N"]) for i in range(params["model"]["n"])]).T

    # Initial control
    us_init             = np.zeros((params["N"],params["model"]["m"]))
...
def waypoint_initial_guess(params):
    ...
    if "z_waypt" not in params:
        params["z_waypt"] = np.zeros(params["model"]["n"])
        
        # Loop through initial conditions
        for i_zi in range(params["mission"]["n_init"]):
            i_state = params["mission"]["zi_idx"][i_zi]

            # Loop through terminal conditions
            for i_zf in range(params["mission"]["n_term"]):
                if params["mission"]["zi_idx"][i_zi] == params["mission"]["zf_idx"][i_zf]:
                    if params["mission"]["zi"][i_zi] != params["mission"]["zf"][i_zf]:
                        params["z_waypt"][i_state] = (params["mission"]["zf"][i_zf] - params["mission"]["zi"][i_zi]) / 2
                    else:
                        params["z_waypt"][i_state] = params["mission"]["zi"][i_zi]

    N1 = params["method"]["N"] // 2
    idx1 = np.arange(1, N1 + 1)

    N2 = params["method"]["N"] - N1
    idx2 = np.arange(N1, params["method"]["N"])

    # Initialize
    zs_init = np.zeros((params["method"]["N"],params["model"]["n"]))

    # Initial state
    for i_state in range(min(params["model"]["n"], len(params["z_waypt"]))):
        if i_state in params["mission"]["zi_idx"] and i_state in params["mission"]["zf_idx"]:
            i_init = np.where(params["mission"]["zi_idx"] == i_state)[0][0]
            i_term = np.where(params["mission"]["zf_idx"] == i_state)[0][0]

            zs_init[idx1-1, i_state]    = np.linspace(params["mission"]["zi"][i_init], params["z_waypt"][i_state], N1)
            zs_init[idx2, i_state]      = np.linspace(params["z_waypt"][i_state], params["mission"]["zf"][i_term], N2)

    # Initial control
    us_init             = np.zeros((params["method"]["N"], params["model"]["m"]))
...
def nonlinear_initial_guess(us_range, problem):
    ...
    params["method"]["dt_init"] = (params["method"]["T_init"] / (params["method"]["N"] - 1)) * np.ones(params["method"]["N"] - 1)
    params["method"]["Ts_init"]  = params["method"]["T_init"] / params["method"]["nondim"]["nt"]
    params["method"]["dts_init"] = params["method"]["dt_init"] / params["method"]["nondim"]["nt"]
    ts_init = np.cumsum(np.concatenate(([0], params["method"]["dts_init"])))
    ...
    # ---- Store results ----
    params["method"]["ts_init"] = ts_init
    params["method"]["zs_init"] = zs_init
    params["method"]["us_init"] = us_init
...
def ctcs_initial_guess(params):
    ...
    params["zs_init"] = np.hstack([params["zs_init"], np.zeros((params["zs_init"].shape[0], params["mission"]["n_ineq"]))])
