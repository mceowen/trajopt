...
def set_ltv_indices(params):
    ...
    params['Sk_ind']    = np.arange(params['Bkp_ind'][-1] + 1, params['Bkp_ind'][-1] + 1 + params['nz'] )
...
def discretize_ctcs(zs_ref, us_ref, dts_ref, problem):
    ...
    Ak = np.zeros((params['nz'], params['nz'], N - 1))
    Bk = np.zeros((params['nz'], params['m'], N - 1))
    Bkp = np.zeros((params['nz'], params['m'], N - 1))
    Sk = np.zeros((params['nz'], 1, N - 1))
