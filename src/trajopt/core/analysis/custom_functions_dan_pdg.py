...
def calc_DCMs(t, z, nu, params):
    veh = params['mission']['vehicle']
    rt = np.array([veh["rt1"], veh["rt2"], veh["rt3"]])
    dcm = DCM(z[7:11])
    return dcm 

def calc_rt_I(t, z, nu, params):
    veh = params['mission']['vehicle']
    rt = np.array([veh["rt1"], veh["rt2"], veh["rt3"]])
    dcm = DCM(z[7:11])
    return dcm.T @ rt

def thrust_mag(t,z,nu,params):
    return np.linalg.norm(nu[:params['model']['dimensions']['m']]);

def ang_rate(t,z,nu,params):
    return (180/np.pi)*np.linalg.norm(z[11:14]);

def compute_tilt(t,z,nu,params):
    return (180/np.pi)*np.arccos(1 - 2*(z[9]**2 + z[10]**2))

def omega_degrees(t,z,nu,params):
    return (180/np.pi)*z[11:14]


scale2d = 0.2
def calc_u_vecs_scale1(t, z, nu, params):
    veh = params['mission']['vehicle']
    rt = np.array([veh["rt1"], veh["rt2"], veh["rt3"]])
    dcm = DCM(z[7:11])
    return -0.3* dcm.T @ nu[:params['model']['dimensions']['m']]

def calc_u_vecs_scale2(t, z, nu, params):
    veh = params['mission']['vehicle']
    rt = np.array([veh["rt1"], veh["rt2"], veh["rt3"]])
    dcm = DCM(z[7:11])
    return -0.3*scale2d* dcm.T @ nu[:params['model']['dimensions']['m']]


def calc_body_vecs_scale1(t, z, nu, params):
    veh = params['mission']['vehicle']
    rt = np.array([veh["rt1"], veh["rt2"], veh["rt3"]])
    dcm = DCM(z[7:11])
    return 0.5 * dcm.T @ np.array([1, 0, 0]);

def calc_body_vecs_scale2(t, z, nu, params):
    veh = params['mission']['vehicle']
    rt = np.array([veh["rt1"], veh["rt2"], veh["rt3"]])
    dcm = DCM(z[7:11])
...
#     mission = params['mission']; #method = params['method']
#     return (z[0] - mission['planet']['r'])/1000;

# def max_q_nonjax(t, z, nu, params):  #dynamic pressure
#     mission = params['mission']; method = params['method']
#     rs = z[0]; vs = z[3]
#     rho = atmosphere_model_nonjax(rs/method['nondim']['nd'], params)
...
#     stateidx = params['indices']['z']['state']
#     Mctrl = method['nondim']['M']['ctrl']['d2nd']
#     aero = nonlinear_aero_nonjax(t, z[stateidx] @ Mstate, nu @ Mctrl,params)
#     L = aero["L"]; D = aero["D"]
