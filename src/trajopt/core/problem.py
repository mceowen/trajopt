...
class Problem:
    ...
    def __init__(self, config, index_map=None):
        ...
        # ------------------------------------------------------------
        # Phases — expand any phase-varying params into length-N arrays
        # ------------------------------------------------------------
    ...
    def _resolve_phases(self, config, index_map):
        ...
        for i, (name, cfg) in enumerate(sorted_phases):
            ...
            phase_params = {k: v for k, v in cfg.items() if k != 'start'}

            for key, val in phase_params.items():
                if not isinstance(self.params.get(key), np.ndarray):
                    self.params[key] = np.zeros(N)
                self.params[key][start:end] = val

            phases.append(AttrDict({'name': name, 'start': start, 'end': end, 'params': phase_params}))
            print(f"  phase {name}: nodes [{start}..{end - 1}], params: {phase_params}")
