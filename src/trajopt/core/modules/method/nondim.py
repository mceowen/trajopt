...
def set_nondim_params(problem, method, base_unit_labels=["m", "s", "kg"]):
    ...
    A = np.vstack([exponents[key] for key in method.scaling["anchor_types"]])
    b = np.log(np.array([val for val in method.scaling["anchor_scales"]]))
    ...
    nd_state = np.array([scales[method.scaling["z_types"][i]] for i in range(n)])
    nd_ctrl  = np.array([scales[method.scaling["u_types"][i]] for i in range(m)])
    ...
    method.nondim["labels"]["state"] = [scale_labels[method.scaling["z_types"][i]] for i in range(n)]
    method.nondim["labels"]["ctrl"]  = [scale_labels[method.scaling["u_types"][i]] for i in range(m)]
